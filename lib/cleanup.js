...
function removeFromFavorites(sessionId, options = {}) {
  const favoritesFile = makeDefaultPaths(options).favoritesFile;
  try {
    const data = JSON.parse(fs.readFileSync(favoritesFile, 'utf8'));
    const ids = Array.isArray(data.ids) ? data.ids.filter((id) => id !== sessionId) : [];
    if (JSON.stringify(ids) === JSON.stringify(data.ids || [])) return { changed: false, failed: [] };
    fs.writeFileSync(favoritesFile, JSON.stringify({ ...data, ids, updated: new Date().toISOString() }, null, 2));
    return { changed: true, failed: [] };
  } catch (error) {
    if (!fs.existsSync(favoritesFile)) return { changed: false, failed: [] };
    return {
      changed: false,
      failed: [{
        path: favoritesFile,
        type: 'pisesh-favorites',
        reason: 'favorites cleanup',
...
function performCleanupPlan(plan, options = {}) {
  ...
  const favorites = sessionDeleted ? removeFromFavorites(plan.session.id, options) : { changed: false, failed: [] };
  failed.push(...favorites.failed);
  return { ok: failed.length === 0, deleted, failed, favoritesChanged: favorites.changed };
}
